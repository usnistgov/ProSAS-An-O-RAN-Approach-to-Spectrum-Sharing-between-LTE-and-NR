...
LTE and NR data for LSTM Model.
"""
# grid search lstm 
import pandas as pd
import numpy as np
...
print('Optimal Configuration for LSTM Model:',cfg_opt)
print('Forecast Accuracy for LSTM Model:',metrics)
# saving the predicted output
d = {'Time':test_time, 'NRB': np.array(predictions).flatten()}
df = pd.DataFrame(data=d)
df.to_csv("LSTM_Model_Prediction_hourly_NR.tsv",index = False, sep = '\t', encoding = 'utf-8-sig')
# Plot chart
...
plt.tight_layout()
plt.savefig('LSTM_Model_GridSearch_1hour_NR.pdf')
plt.show()
