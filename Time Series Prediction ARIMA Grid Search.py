...
LTE and NR data for Autoregressive Integrated Moving Average (ARIMA) Model.
"""
import pandas as pd
import numpy as np
import itertools
...
def sarima_configs(seasonal_P = [0], seasonal_D = [0], seasonal_Q = [0], seasonal=[0]):
	models = list()
	# define config lists
	p_params = [0, 1, 2, 3]
	d_params = [0, 1, 2, 3]
...
if __name__ == '__main__':
    ...
    print('Optimal Configuration for ARIMA Model:',cfg_opt)
    print('Forecast Accuracy for ARIMA Model:',metrics)
    # saving the predicted output
    d = {'Time':test_time, 'NRB': pred}
    df = pd.DataFrame(data=d)
    df.to_csv("ARIMA_Model_Prediction_hourly_NR.tsv",index = False, sep = '\t', encoding = 'utf-8-sig')
    # Plot chart
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('ARIMA_Model_GridSearch_1hour_NR.pdf')
    plt.show()