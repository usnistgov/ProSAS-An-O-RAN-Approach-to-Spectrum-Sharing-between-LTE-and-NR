...
LTE and NR data for Convolutional-LSTM Model.
"""
# grid search ConvLSTM
import pandas as pd
import numpy as np
...
print('Optimal Configuration for ConvLSTM Model:',cfg_opt)
print('Forecast Accuracy for ConvLSTM Model:',metrics)
# saving the predicted output
d = {'Time':test_time, 'NRB': np.array(predictions).flatten()}
df = pd.DataFrame(data=d)
df.to_csv("ConvLSTM_Model_Prediction_hourly.tsv",index = False, sep = '\t', encoding = 'utf-8-sig')
# Plot chart
...
plt.tight_layout()
plt.savefig('ConvLSTM_Model_GridSearch_1hour.pdf')
plt.show()
